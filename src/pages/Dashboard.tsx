...
export function Dashboard({ salonId, onLogout }: DashboardProps) {
  ...
  const { isAccessAllowed, status, daysRemaining, loading: loadingSubscription } = useSubscription(salonId);
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 flex flex-col justify-between">
        <div className="w-full">
          <Payments salonId={salonId} currentStatus={status} daysRemaining={daysRemaining} />
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-white dark:bg-slate-900 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800 p-6 flex flex-col justify-between shrink-0">
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-2 px-2">
            ...
            <span className="font-black text-base text-slate-900 dark:text-white uppercase tracking-wider">My Saloom</span>
          </div>
      ...
      <main className="flex-1 overflow-y-auto">
        {activeTab === 'dashboard' && (
          <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
              <div>
                <h1 className="text-xl font-black text-slate-900 dark:text-white">Fila de Espera</h1>
                <p className="text-xs text-slate-500 mt-0.5">Gerencie os clientes agendados e os que estão na bicha.</p>
              </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                <CalendarIcon size={16} className="text-saloom-500" /> Próximos Clientes
              </h3>

              {loadingBookings ? (
                <div className="text-center py-12 text-xs font-semibold text-slate-400">A carregar bicha atual...</div>
              ) : bookings.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {bookings.map((booking) => (
                    <div key={booking.id} className="py-4 flex items-center justify-between first:pt-0 last:pb-0">
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 dark:text-white">{booking.customer_name}</h4>
                        <p className="text-xs text-slate-400 mt-0.5">{booking.service_type} • {booking.estimated_time || 'Sem hora'}</p>
                      </div>
        ...
        {activeTab === 'payments' && <Payments salonId={salonId} currentStatus={status} daysRemaining={daysRemaining} />}
      </main>
